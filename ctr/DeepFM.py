...
def parse_args():
    ...
    parser.add_argument('--dataset', nargs='?', default='WN18',
...
class NeuralFM(BaseEstimator, TransformerMixin):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            # init
            self.saver = tf.train.Saver()#保存训练好的模型
    ...
    def get_random_block_from_data(self, data, batch_size):  # 生成一个随机的训练数据块
        start_index = np.random.randint(0, len(data['Y']) - batch_size)
        X , Y = [], []
        # forward get sample向前获取样本
        i = start_index
    ...
    def train(self, Train_data, Validation_data, Test_data):  # fit a dataset
        ...
        for epoch in range(self.epoch):
            ...
            if self.verbose > 0 and epoch%self.verbose == 0:
                 print('epoch:',epoch+1, 'train_loss:', train_result, 'validation_loss:',valid_result, 'test_loss:',test_result)
